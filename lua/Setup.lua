...
function GameInfoManager:_camera_event(event, key, data)
	if event == "create" then
		if not self._cameras[key] then
			local type = GameInfoManager.CAMERAS[tostring(tostring(data.unit:name():key()))]
			local is_drone = type == "drone"
			self._cameras[key] = { unit = data.unit, is_drone = is_drone }
			self:_listener_callback("camera", event, key, self._cameras[key])

			if is_drone then
				self:_camera_event("set_enabled", key, { enabled = true })
	...
	elseif self._cameras[key] then
		if event == "set_active" then
			self._cameras[key].active = self._cameras[key].is_drone or data.active
		elseif event == "set_enabled" then
			self._cameras[key].enabled = data.enabled
