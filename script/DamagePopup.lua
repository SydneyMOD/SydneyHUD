if RequiredScript == "lib/units/enemies/cop/copdamage" then
	...
	function CopDamage:_process_popup_damage(attack_data)
		CopDamage.DMG_POPUP_SETTING = 2 --SETTING
	...
	function CopDamage:show_popup(damage, dead, headshot, color_id)
		if managers.waypoints then
			...
			local waypoint_color = color_id and ((color_id == 5 and false) and Color.White or tweak_data.chat_colors[color_id]) or Color("FFFF00")
			local waypoint_duration = 3
			if waypoint and not waypoint:is_deleted() then
				...
				local params = {
					...
					component_order = { 1 == 1 and { "icon", "label" } or { "label", "icon" } , { "duration" } }
				}
